...
- Assumes input CSV columns include:
    ts_event_ms, ts_recv_ns, bid_p1, bid_q1, ... bid_p20, bid_q20, ask_p1, ask_q1, ... ask_p20,
ask_q20
...
namespace md::l2
{
  ...
  namespace
  {
        ...
        const int lvl = i + 1;
        {
          ...
          if ( m.bid_p[i] < 0 || m.bid_q[i] < 0 ) {
            throw std::runtime_error(
                "Missing required bid columns for level " + std::to_string(lvl));
          }
        }
        {
          ...
          if ( m.ask_p[i] < 0 || m.ask_q[i] < 0 ) {
            throw std::runtime_error(
                "Missing required ask columns for level " + std::to_string(lvl));
          }
        }
      }
      return m;
    ...
    void init_record_sentinels(Record& rec)
    {
      ...
      rec.ts_recv_ns = 0;
      for ( auto& l : rec.bids )
        l = Level{kBidNullPriceQ, kNullQtyQ};
      for ( auto& l : rec.asks )
        l = Level{kAskNullPriceQ, kNullQtyQ};
    }
        ...
        std::uint64_t& bad_rows)
    {
      ...
      if ( cm.ts_event_ms >= 0 && cm.ts_event_ms < static_cast<int>(row.size()) ) {
        std::int64_t t = 0;
        if ( parse_i64(row[cm.ts_event_ms], t) )
          rec.ts_event_ms = t;
      }
      ...
      for ( int i = 0; i < kDepth; ++i ) {
        ...
        if ( bp >= 0 && bq >= 0 && bp < static_cast<int>(row.size()) &&
             bq < static_cast<int>(row.size()) ) {
          std::int64_t px = 0, qy = 0;
          if ( parse_fixed(row[bp], kPriceScale, px) && parse_fixed(row[bq], kQtyScale, qy) &&
               px > 0 && qy > 0 ) {
            rec.bids[i] = Level{px, qy};
          }
        }

        if ( ap >= 0 && aq >= 0 && ap < static_cast<int>(row.size()) &&
             aq < static_cast<int>(row.size()) ) {
          std::int64_t px = 0, qy = 0;
          if ( parse_fixed(row[ap], kPriceScale, px) && parse_fixed(row[aq], kQtyScale, qy) &&
               px > 0 && qy > 0 ) {
    ...
    void atomic_rename(const fs::path& tmp, const fs::path& final)
    {
      ...
      if ( ec ) {
        throw std::runtime_error(
            "Failed to create output directory: " + final.parent_path().string());
      }
      ...
      if ( ec ) {
        throw std::runtime_error(
            "Failed to rename tmp->final: " + tmp.string() + " -> " + final.string() + " : " +
            ec.message());
      }
    }
  ...
  void convert(const std::string& input_path, const std::string& output_path)
  {
    ...
    if ( payload_sz % sizeof(Record) != 0 || expected != count ) {
      throw std::runtime_error(
          "Output size mismatch: file_sz=" + std::to_string(file_sz) + " expected_records=" +
          std::to_string(expected) + " header_records=" + std::to_string(count));
    }
