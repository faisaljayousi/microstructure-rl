...
static std::vector<std::string> discover_snaps_from_processed_root()
{
  ...
  if ( root.empty() ) {
    throw std::runtime_error("DATA_PROCESSED_ROOT not set. Load .env or export it in the shell.");
  }
...
static void RunReplayBench(benchmark::State& state, TouchFn touch)
{
  ...
  state.SetBytesProcessed(
      static_cast<int64_t>(records) * static_cast<int64_t>(sizeof(md::l2::Record)));
  ...
  state.counters["workset_MiB"] = benchmark::Counter(
      static_cast<double>(ws_bytes) / (1024.0 * 1024.0),
      benchmark::Counter::kAvgThreads);
  state.counters["n_files"] =
      benchmark::Counter(static_cast<double>(snaps.size()), benchmark::Counter::kAvgThreads);
}

// -------------------------
// Benchmarks
// -------------------------
static void BM_Replay_TOB(benchmark::State& state) { RunReplayBench(state, touch_tob); }

static void BM_Replay_FullTouch(benchmark::State& state) { RunReplayBench(state, touch_full); }
